...
) : COLOR
{
    ...
    //float4 Diff =  max( 0, ndl ) * k_d * I_d;d;
    ...
    float4 bcClr = tex2D(BrushSampler, brhTex);
    bcClr.rgb *= bcClr.a;

    resultColor += bcClr;

    float f = 1.0 / exp(depth.y * fogDensity);
    return lerp(fogColor, resultColor,  f);
}
...
{ 
    ...
    brhTex = float2(iPos.x - brushPos.x + brushSize * 0.5, iPos.z - brushPos.y + brushSize * 0.5);
    brhTex /= brushSize;
}
